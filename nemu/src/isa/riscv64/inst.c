...
//    assign IntALUct=(func3==3'b011)?5'b01010:{1'b0,func7[5]&((|func3)|(opcode[5])),func3};
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11",slli,I,R(dest)=src1<<BITS(src2,5,0));//逻辑左移  shamt
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11",sra,R,R(dest)= ((sword_t)src1>>BITS(src2, 4, 0)));//c语言中有符号数右移>>为算术右移，无符号数右移>>为逻辑右移
