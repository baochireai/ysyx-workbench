...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11",slli,I,R(dest)=src1<<BITS(src2,5,0));//逻辑左移
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11",slliw,I,R(dest)=SEXT(BITS(src1<<BITS(src2,5,0),31,0),32));//逻辑左移

  INSTPAT("0000000 ????? ????? 101 ????? 00100 11",srli,I,R(dest)= (src1)>>BITS(src2, 5, 0));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11",srliw,I,R(dest)= SEXT(BITS(BITS(src1,31,0)>>BITS(src2, 5, 0),31,0),32));

  INSTPAT("0100000 ????? ????? 101 ????? 00100 11",srai,I,R(dest)= ((sword_t)src1)>>BITS(src2, 5, 0));
  INSTPAT("0100000 ????? ????? 101 ????? 0qqq0 11",sraw,R,R(dest)= ((sword_t)SEXT(BITS(src1,31,0),32))>>BITS(src2, 5, 0));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11",sraiw,I,R(dest)= ((sword_t)SEXT(BITS(src1,31,0),32))>>BITS(src2, 5, 0));
