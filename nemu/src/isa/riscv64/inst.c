...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11",sltiu,I,R(dest)=(src1<src2));//slti 变010
//    assign IntALUct=(func3==3'b011)?5'b01010:{1'b0,func7[5]&((|func3)|(opcode[5])),func3};
  ...
  //整数运算指令-寄存器-寄存器
  //相同点 指令类型都是R 操作码一样  （w optcode[3]:1  u funct3[0]:1）
 ...
 //条件跳转指令 (寄存器-寄存器)
  ...
  //整数运算指令-立即数
  INSTPAT("??????? ????? ????? ??? ????? 01101 11",lui,U,R(dest)=src1);//读取立即数
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = src1 + s->pc);

  // 存储器写指令 funct3[1:0]字节数 地址计算（寄存器+imm）
